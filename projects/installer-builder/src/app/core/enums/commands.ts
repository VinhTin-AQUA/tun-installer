export enum Commands {
    WRITE_DATA_TO_EXE_COMMAND = 'write_data_to_exe_command',
    LOAD_HTML_PAGES_COMMAND = 'load_html_pages_command',
    PREVIEW_INSTALLER_UI_COMMAND = 'preview_installer_ui_command',
    SAVE_INSTALLER_CONFIG_COMMAND = 'save_installer_config_command',
    UPDATE_WORKING_CONFIG_COMMAND = 'update_working_config_command',
    LOAD_WORKING_CONFIG_COMMAND = 'load_working_config_command',
    LOAD_INSTALLER_DOCUMENT_CONFIG_COMMAND = 'load_installer_document_config_command'
}
