...
pub fn compress_installer() -> Result<()> {
...
pub fn extract_installer() -> Result<()> {
    // let exe = std::env::current_exe()?;
    let exe = PathBuf::from("data/exe_template.exe");
...
pub fn progressbar(rel_path: &String, file_sizes: &u64, total_size: &u64) {
    if total_size == &0 {
        return;
    }
...
fn embed_folders_into_exe(exe_path: &Path, folders: &[PathBuf]) -> Result<()> {
...
fn extract_root(exe_path: &Path, output: &Path, target_root: &str) -> Result<()> {
